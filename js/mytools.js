	...
	**/		
	function checkAll(type,value){ 		 //type是你要验证的类型（自定），value验证该值
        ...
        case 'postId':  //邮编：不能以0开头6位数字
			...
			break; 
		case 'username':  //用户名以数字、字母、下划线组合，不能以数字开头，用户名的长度不得少于6个字符，不得多于12个字符
			if((/^[a-zA-Z_]\w{5,12}$/).test(value)){   
				return true;   
			}else{  
				return false;
			}  
	...
	**/		
	function drup(obj){
		obj.onmousedown=function(){
			...
			let x=e.offsetX;//发生事件的地点在事件源元素的坐标系统中的X坐标、Y坐标
			let y=e.offsetY;
